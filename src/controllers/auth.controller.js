...
exports.signup = async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            return res.status(400).json({ type: 'error', message: 'User not created' });
        }
        ...
        if(error.message.includes("duplicate key error") && error.message.includes("email")){
            return res.status(400).json({type: "error", message: "User with that email already exists"})
            // console.log("User with that email already exists")
        }     
        if(error.message.includes("duplicate key error") && error.message.includes("username")){
            return res.status(400).json({type: "error", message: "User with that username already exists"})
            // console.log("User with that email already exists")
        }     
...
exports.login = async (req, res) => {
    try {
        ...
        return res.status(200).json({type: "success", message: "Login successful", ...user._doc, accessToken});
